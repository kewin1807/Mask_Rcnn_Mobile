...
def mobilenetv1_graph(inputs, architecture, alpha=1.0, depth_multiplier=1, train_bn=False):
    ...
    C1 = x = _depthwise_conv_block(
        x, 64, alpha, depth_multiplier, block_id=1, train_bn=train_bn)

    # Stage 2
    x = _depthwise_conv_block(x, 128, alpha, depth_multiplier, strides=(
        2, 2), block_id=2, train_bn=train_bn)
    C2 = x = _depthwise_conv_block(
        x, 128, alpha, depth_multiplier, block_id=3, train_bn=train_bn)  # Input Resolution: 56 x 56

    # Stage 3
    x = _depthwise_conv_block(x, 256, alpha, depth_multiplier, strides=(
        2, 2), block_id=4, train_bn=train_bn)
    C3 = x = _depthwise_conv_block(
        x, 256, alpha, depth_multiplier, block_id=5, train_bn=train_bn)  # Input Resolution: 28 x 28
    ...
    C4 = x = _depthwise_conv_block(
        x, 512, alpha, depth_multiplier, block_id=11, train_bn=train_bn)  # Input Resolution: 14 x 14

    # Stage 5
    x = _depthwise_conv_block(x, 1024, alpha, depth_multiplier, strides=(
        2, 2), block_id=12, train_bn=train_bn)
    C5 = x = _depthwise_conv_block(
        x, 1024, alpha, depth_multiplier, block_id=13, train_bn=train_bn)  # Input Resolution: 7x7
    return [C1, C2, C3, C4, C5]
...
def _inverted_residual_block(inputs, filters, kernel, t, strides, n, alpha, block_id, train_bn):
    ...
    x = _bottleneck(inputs, filters, kernel, t, strides,
                    False, alpha, block_id, train_bn)

    for i in range(1, n):
        block_id += 1
        x = _bottleneck(x, filters, kernel, t, 1, True,
                        alpha, block_id, train_bn)
...
def mobilenetv2_graph(inputs, architecture, alpha=1.0, train_bn=False):
    ...
    x = _conv_block(inputs, 32, alpha, (3, 3), strides=(
        2, 2), block_id=0, train_bn=train_bn)                      # Input Res: 1
    C1 = x = _inverted_residual_block(
        x, 16,  (3, 3), t=1, strides=1, n=1, alpha=1.0, block_id=1, train_bn=train_bn)  # Input Res: 1/2
    C2 = x = _inverted_residual_block(
        x, 24,  (3, 3), t=6, strides=2, n=2, alpha=1.0, block_id=2, train_bn=train_bn)  # Input Res: 1/2
    C3 = x = _inverted_residual_block(
        x, 32,  (3, 3), t=6, strides=2, n=3, alpha=1.0, block_id=4, train_bn=train_bn)  # Input Res: 1/4
    x = _inverted_residual_block(x, 64,  (3, 3), t=6, strides=2, n=4,
                                 alpha=1.0, block_id=7, train_bn=train_bn)  # Input Res: 1/8
    C4 = x = _inverted_residual_block(
        x, 96,  (3, 3), t=6, strides=1, n=3, alpha=1.0, block_id=11, train_bn=train_bn)  # Input Res: 1/8
    x = _inverted_residual_block(x, 160, (3, 3), t=6, strides=2, n=3,
                                 alpha=1.0, block_id=14, train_bn=train_bn)  # Input Res: 1/16
    C5 = x = _inverted_residual_block(
        x, 320, (3, 3), t=6, strides=1, n=1, alpha=1.0, block_id=17, train_bn=train_bn)  # Input Res: 1/32
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        if config.BACKBONE in ["resnet50", "resnet101"]:
            ...
            _, C2, C3, C4, C5 = mobilenetv1_graph(
                input_image, config.BACKBONE, alpha=1.0, train_bn=config.TRAIN_BN)
        elif config.BACKBONE in ["mobilenetv2"]:
            _, C2, C3, C4, C5 = mobilenetv2_graph(
                input_image, config.BACKBONE, alpha=1.0, train_bn=config.TRAIN_BN)
        # Top-down Layers
    ...
    def compile(self, learning_rate, momentum):
        ...
        reg_losses = [
            KR.l2(self.config.WEIGHT_DECAY)(w) /
